 ...
 * `DebtService` e `BudgetService` já usam o backend Spring Boot via
 * `HttpAccountService`/`HttpCategoryService`/`HttpIncomeService`/
 * `HttpExpenseService`/`HttpDebtService`/`HttpBudgetService`; os demais
 * domínios continuam em `localStorage` até serem integrados em etapas
 * futuras. Quando isso acontecer, basta trocar o `useClass` de cada
